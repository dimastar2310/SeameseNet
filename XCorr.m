...
classdef XCorr < dagnn.Layer
    ...
    methods
        function outputs = forward(obj, inputs, params)
            if obj.bias %3 variables indicator or less
            ...
            if obj.bias
                outputs{1} = cross_corr(inputs{1:3}); %z x and c ,x is the bigger picture
            else
                outputs{1} = cross_corr(inputs{1:2}, []);
            end
        end

        function [derInputs, derParams] = backward(obj, inputs, params, derOutputs)%loss function has one arguments and its last layer
            if obj.bias
                assert(numel(inputs) == 3, 'three inputs are needed');
            else
                assert(numel(inputs) == 2, 'two inputs are needed');
